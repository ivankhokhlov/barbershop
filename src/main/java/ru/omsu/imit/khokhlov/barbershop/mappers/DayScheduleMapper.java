...
public interface DayScheduleMapper {
    @Insert("INSERT INTO dayschedule (curDate,master_id) VALUES "
            +"(#{dayschedule.curDate},#{dayschedule.master.user.id})")
    @Options(useGeneratedKeys = true, keyProperty = "dayschedule.id")
    Integer insert(@Param("dayschedule") DaySchedule dayschedule);
    @Select("SELECT id,curDate,master_id FROM dayschedule WHERE id = #{id}")
    @Results({
    ...
    @Select("SELECT id,curDate,master_id FROM dayschedule WHERE master_id = #{master.user.id}")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "master", column = "master_id", javaType = Master.class,
                    one = @One(select = "ru.omsu.imit.khokhlov.barbershop.mappers.MasterMapper.getById", fetchType = FetchType.LAZY)),
    ...
    @Select("SELECT id,curDate FROM dayschedule WHERE master_id = #{master.user.id} AND curDate >= #{dateStart} AND curDate <= #{dateEnd}")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "master", column = "master_id", javaType = Master.class,
                    one = @One(select = "ru.omsu.imit.khokhlov.barbershop.mappers.MasterMapper.getById", fetchType = FetchType.LAZY)),
    ...
    List<DaySchedule> getByMasterAndDate(@Param("master") Master master, @Param("dateStart")LocalDate dateStart, @Param("dateEnd")LocalDate dateEnd);

    @Select("SELECT id,curDate FROM dayschedule WHERE curDate >= #{dateStart} AND curDate <= #{dateEnd}")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "master", column = "master_id", javaType = Master.class,
                    one = @One(select = "ru.omsu.imit.khokhlov.barbershop.mappers.MasterMapper.getById", fetchType = FetchType.LAZY)),
